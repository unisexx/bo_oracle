<?php
Class Welfare_community extends  Act_Controller{
	...
	function index(){
		...
		WHERE ACT_ORGANIZATION_MAIN.UNDER_TYPE = 3 ".@$condition." 
		...
		$this->template->build('welfare_community/index',$data);
	}
...
// 		
		if($id != ""){
			...
			$data['service_com_select'] = $this->orgsub->where("question_name = 'service_com' and organ_id = ".$id)->get(FALSE,TRUE);
			$data['pcommunity_select'] = $this->orgsub->where("question_name = 'ProcessCommunity' and organ_id = ".$id)->get(FALSE,TRUE);
		}
		...
		$this->template->build('welfare_community/form',$data);
	}
	...
	function save(){
		...
		redirect('act/welfare_community');
