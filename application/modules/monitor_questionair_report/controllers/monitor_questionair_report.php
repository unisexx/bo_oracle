<?php		
class Monitor_questionair_report extends Monitor_Controller
{
	...
	function index()
	{
		...
		$condition .= @$_GET['pprovince_id'] != '' ? " AND PROVINCEID=".@$_GET['pprovince_id']." AND PROVINCEID <> 2 AND PROVINCEID > 0 " : " AND PROVINCEID <> 2 AND PROVINCEID > 0 ";
		$sql = " SELECT COUNT(*) 
		FROM MT_QUESTIONAIR 
	...
	function test_index()
	{
		...
		$this->template->build('test_index',$data);		
	}
