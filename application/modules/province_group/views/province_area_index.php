...
<div id="btnBox">
  <input type="button" title="เพิ่มรายการ" value=" " onclick="document.location='c_province_area/form'" class="btn_add"/>
</div>
...
<tr <? if($rowStyle =='')$rowStyle = 'class="odd"';else $rowStyle = "";echo $rowStyle;?> >
  <td><?=$i;?></td>
  <td onclick="window.location='c_province_area/form/<?=$row['id'];?>'"><?=$row['title'];?> </td>
  <td>
  	<input type="button" name="button" id="button" value="x" class="btn_delete" onclick="confirmDelete('c_province_area/delete/<?=$row['id'];?>/<?=$page;?>','<?php echo NOTICE_CONFIRM_DELETE?>');">  	
  </td>
  </tr>
<tr>
<? 	
