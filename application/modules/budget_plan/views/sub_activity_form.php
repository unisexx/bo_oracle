...
$().ready(function() {
	...
	$("#frmStrategy").validate({
		...
		messages: {
			...
			mainactid:"กรุณาเลือก<?=$lv_title[10];?>"
		}
    });
    ...
    $("select[name=budgetpolicyid]").live('change',function(){
    	$.post('budget_plan/reload_list',{
    		'target_lv' : 'mainactid',
    		'pid' : $(this).val()    		
    	},function(data)
    	{
    		$("#td_mainactid").html(data);
    	})
    })	
});					
</script>
...
<table class="tbadd">  
  ...
  <tr>
  	...
  	<td id="td_budgetpolicyid">
  		<?php echo form_dropdown('budgetpolicyid',get_option("id","title","cnf_strategy"," syear=".$budgetyear." AND  pid=".$prow['productivityid']),@$prow['budgetpolicyid'],'','-- '.$lv_title[9].' --');?>
  	</td>
  </tr>
  <tr>
  	<th><?=$lv_title[10];?></th>
  	<td id="td_mainactid">
  		<?php echo form_dropdown('mainactid',get_option("id","title","cnf_strategy"," syear=".$budgetyear." AND  pid=".$prow['pid']),@$prow['id'],'','-- '.$lv_title[10].' --');?>
  	</td>
  </tr>
  <tr>
    <th width="18%"> 
     ...
     <input type="hidden" name="lv" id="lv" value="11">     
     <input type="hidden" name="createdate" id="createdate" value="<? if(@$row['createdate']>0)echo $row['createdate'];else echo date("Y-m-d");?>">      
     </td>
  </tr>
  <th width="18%"> 
		ภารกิจ : <span class="Txt_red_8">*</span>
  </th>
  <td> 
     	<input type="radio" value="ภารกิจพื้นฐาน" name="missiontype" id="missiontype" <? if(@$row['missiontype']=='ภารกิจพื้นฐาน')echo 'checked="checked"';?>>ภารกิจพื้นฐาน
        &nbsp;&nbsp;
        <input type="radio" value="ภารกิจยุทธศาสตร์" name="missiontype" id="missiontype" <? if(@$row['missiontype']=='ภารกิจยุทธศาสตร์')echo 'checked="checked"';?>>ภารกิจยุทธศาสตร์
  </td>
</table>
 </div>
