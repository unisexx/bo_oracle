		...
		$('.change_status').live('change', function (){
			...
			if(confirm('ท่านต้องการเปลี่ยนสถานะ การใช้งาน')) {	
				if ($("[name='status_id["+ref_id+"]']").is(':checked')){
					...
					$.get('<? echo site_url(); ?>mds_set_assessment/change_status',
...
<h3>ตั้งค่า หัวข้อประเด็นการประเมินผล</h3>
<form method="GET">
<div id="search">
...
<tr>
  ...
  <th align="left">ชื่อประเด็นการประเมินผล</th>
...
?>
	<tr <? if($rowStyle =='')$rowStyle = 'class="odd"';else $rowStyle = "";echo $rowStyle;?> >
			...
			<td onclick="window.location='<?php echo @$urlpage;?>/form/<?=$item['id'];?>'"><? echo $item['ass_name'];?></td>
			<td>
