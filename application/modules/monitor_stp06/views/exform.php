...
<center><h3>แบบประเมินผลการดำเนินงานตาม<?=$topic['title']?> ของ สำนักงานพัฒนาสังคมและความมั่นคงของมนุษย์จังหวัด</h3></center>
		...
		<?php foreach($maintitled as $maintitle):?>
			...
			<?php if($maintitle['form_type'] == 1): ?>
				...
				<?php foreach($subtitled as $subtitle):
					//$this->db->debug = true;
					//$r2 = $this->r2->get_row('mt_subtitle_id',$subtitle['id']);
				?>
				<tr>
					<td>
						<?=$subtitle['subtitle']?>
			...
			<?php elseif($maintitle['form_type'] == 3):
				//$r2 = $this->r2->get_row('mt_maintitle_id',$maintitle['id']);
			?>
				<tr>
					<td colspan="3">
...
<!-- <div id="btnBoxAdd">
	<input type="hidden" name="workgroupid" value="<?=login_data('workgroupid')?>">
	<input type="hidden" name="id" value="<?=$r1['id']?>">
	<input type="hidden" name="mt_topic_id" value="<?=$topic['id']?>">
	<input type="hidden" name="province_id" value="<?=login_data("user_province_id")?>">
