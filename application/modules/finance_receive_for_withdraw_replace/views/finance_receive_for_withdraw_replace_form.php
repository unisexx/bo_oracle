...
<tr>
  ...
  <td>
    <input name="book_no" type="text" id="book_no" size="40" value="<?php echo @$result['book_no'];?>"/>
  ลงวันที่ <input name="book_date" type="text" id="book_date" size="10" class="datepicker" value="<?php if(@$result['book_date'] > 0)echo stamp_to_th($result['book_date']);?>" />
  </td>
</tr>
<tr>
  <th>เลขที่ส่วนการคลังรับ </th>
  <td>
    <input name="finance_no" type="text" id="finance_no" size="40" value="<?php echo @$result['finance_no'];?>"/>
    ลงวันที่ <input name="finance_date" type="text" id="finance_date" size="10" class="datepicker" value="<?php if(@$result['finance_date'] > 0)echo stamp_to_th($result['finance_date']);?>" /></td>
</tr>
<tr>
  <th>เรื่อง</th>
  ...
  echo @form_dropdown('mainactid',get_option('id','title',$option),@$result['mainactid'],'','-- เลือกกิจกรรมหลัก --')?>	
   ...
   echo @form_dropdown('subactivityid',get_option('id','title',$option),$result['subactivityid'],'','-- เลือกกิจกรรมย่อย --')?>
	...
	<tr>
		...
		<td><input type="text" name="receive_date" class="datepicker"></td>
	</tr>
</table>
