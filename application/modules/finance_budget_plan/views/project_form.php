...
$(document).ready(function(){
	...
	$('select[name=divisionid]').live('change',function(){
		...
		if(divisionid != 0){
			...
			$("<img src='images/loadingAnimation.gif' style='margin:0 0 0 5px;' />").appendTo(".tdwg");
	...
	$("form").validate({
		...
		messages:{
			...
			projecttitle:"กรุณาระบุข้อมูลด้วย"
		}
	});
	
	$(".eptotal").keypress(function(){
		var ep_value = Number($(this).val().replace(/[^0-9\.]+/g,""));
		var percent = Number($(this).closest("tr").find(".ep").val().replace(/[^0-9\.]+/g,""));
		var percent_value = percent > 0 ? ep_value * (percent/100) : 0;
		var ep_net = ep_value - percent_value;
		//alert(ep_value);
		//alert(percent);
		//alert(percent_value);
		//alert(ep_net);
		$(this).closest("tr").find("#ep_percent").val(new NumberFormat(percent_value).toFormatted());
		$(this).closest("tr").find("#ep_net").val(new NumberFormat(ep_net).toFormatted()); 
		CalculateSummary();
	})
...
function summary()
{
	...
	var n_summary = 0;
		...
		$(".ep_percent_total").each(function(){
			p_summary+=  Number($(this).val().replace(/[^0-9\.]+/g,""));
		});
		$(".ep_net_total").each(function(){
			n_summary+= Number($(this).val().replace(/[^0-9\.]+/g,""));
		});
		$("#summary").html(new NumberFormat(summary).toFormatted());
		$("#percent_summary").html(new NumberFormat(p_summary).toFormatted());
		$("#net_summary").html(new NumberFormat(n_summary).toFormatted());
}
function CalculateSummary(pBudgetTypeID){
	summary();
	
	
	var list_bgtype = $("#list_bgtype").val();
	var mySplitResult = list_bgtype.split("|");

	for(i = 0; i < mySplitResult.length; i++){
	var total=0;
	var p_total=0;
	var n_total = 0;	
		pBudgetTypeID=mySplitResult[i];
	    $(".eptotal"+pBudgetTypeID).each(function() {
			total += Number($(this).val().replace(/[^0-9\.]+/g,""));
		});
		$(".ep_percent_total"+pBudgetTypeID).each(function() {
				p_total += Number($(this).val().replace(/[^0-9\.]+/g,""));
		});
		$(".ep_net_total"+pBudgetTypeID).each(function() {
				n_total += Number($(this).val().replace(/[^0-9\.]+/g,""));
		});
		$(".tdbgtotal"+pBudgetTypeID).html(new NumberFormat(total).toFormatted());
		$(".tdbg_percent_total"+pBudgetTypeID).html(new NumberFormat(p_total).toFormatted());
		$(".tdbg_net_total"+pBudgetTypeID).html(new NumberFormat(n_total).toFormatted());
	}	
...
<tr>
  ...
  <td class="tddp"><?php echo @form_dropdown('departmentid',get_option('id','title',"cnf_department "),$division['departmentid'],'','-- เลือกกรม --')?></td>
</tr>
<tr>
  <th>หน่วยงาน<span class="Txt_red_12"> *</span></th>
  <td class="tddv"><?php echo @form_dropdown('divisionid',get_option('id','title',"cnf_division "),$division['id'],'','-- เลือกหน่วยงาน --')?></td>
</tr>
<tr>
  <th>กลุ่มงาน<span class="Txt_red_12"> *</span></th>
  <td class="tdwg"><?php echo @form_dropdown('workgroupid',get_option('id','title',"cnf_workgroup WHERE DIVISIONID=".$division['id']),$project['workgroup_id'],'','-- เลือกกลุ่มงาน --')?></td>
</tr>
<tr>
  <th>โครงการ<span class="Txt_red_12"> *</span></th>
  <td><input type="text" name="projecttitle" id="projecttitle" value="<?=@$project['projecttitle'];?>" size="90"></td>
</tr>
</table>
