	...
	$('select[name=departmentid]').live('change',function(){
		...
		if(departmentid != 0){
			...
			$.post('ajax/load_division_list',{
				...
				'canaccessall' : '<?=login_data("fn_access_all");?>'
			},function(data){
				$(".tddv").html(data);
			})
		}
...
<tr>
  <th>กรม</th>
  <td class="tddp"><?php echo @form_dropdown('departmentid',get_option('id','title',"cnf_department "," financeuse = 'on' "),$self['departmentid'],'','-- เลือกกิจกรรมย่อย --')?></td>
</tr>
<tr>
  <th>หน่วยงาน</th>
  <td class="tddv"><?php echo @form_dropdown('divisionid',get_option('id','title',"cnf_division "),$self['divisionid'],'','-- เลือกกิจกรรมย่อย --')?></td>
</tr>
</table>
