...
<table class="tbadd">
	<tr>
	  ...
	  <td>
	  	<? // echo form_dropdown('fnyear',get_option("fnyear","fnyear+543 as fn","fn_strategy group by fnyear"),@$self['fnyear'],'','-- เลือกปีงบประมาณ --')?>
	  	<?
	  	$min_year = 2012;
		$max_year = date("m") < 9 ? date("Y") : date("Y") + 1;
	  	?>
	  	<select name="fnyear">
	  		<? for($i=$min_year;$i<=$max_year;$i++){ ?>
	  			<option value="<?=$i;?>" <? if($i==@$self['fnyear'])echo 'selected="selected"';?>><?=$i+543;?></option>
	  		<? } ?>
	  	</select>
	  	<?php 
	  	?>
	...
	<tr>
	  <th><label for="fid-full_name3">ช่วงแผนงบประมาณ <span class="Txt_red_12"> *</span></label></th>
	  <td>
