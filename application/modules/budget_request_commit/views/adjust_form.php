...
$(document).ready(function(){
	...
	$(".tr_sum_request > td").each(function(){
		...
		td_sum_request.append(new NumberFormat(summ).toFormatted())						
	})
	$(".tr_sum_adjust > td").each(function(){
		...
		tr_sum_adjust.append(new NumberFormat(summ).toFormatted())
	})
...
<tr>
      ...
      ?>
	  <th align="center"><?=$bType['title'];?></th>
      <? }
	  }
	  ?>
  </tr>
...
<tr>
  ...
  <td class="td_summary_row" nowrap="nowrap" align="right"></td>
      ...
      ?>  
		  ...
		  <? if($totalp > 0 )echo number_format($totalp,2);?>	
...
<? } ?>
  <tr class="tr_sum_request">
  		<th>รวมยอดที่ขอ</th>
  		<th></th>
		<? for($i=0;$i<=($totalColumn-1);$i++) { ?>
			  <td class="col<?=$i+1;?>" align="right">
			  	&nbsp;			  	
			  </td>
  		<? } ?>
  </tr>
  <tr class="tr_sum_adjust">
  		<th>รวมยอดที่ปรับ</th>
  		<th></th>
		<? for($i=0;$i<=($totalColumn-1);$i++) { ?>
			  <td class="col<?=$i+1;?>" align="right">
...
<div style="padding-left:35%; padding-top:10px;">
   <? if($step>1 ){ ?><label  style="width:350px">
   	<input type="checkbox" id="chkfollow" name="chkfollow" value="1"   /> เลือกถ้าขั้นตอนการปรับปรุงครั้งนี้ไม่่มีรายการเปลี่ยนแปลง </label><br /><br /><? } ?>
  <input type="submit" name="button22" id="button22" value="" class="btn_save"  />
  &nbsp;
