...
function GetMainActWithdraw($pWYear = FALSE,$pWMonth = FALSE,$pMainActID = FALSE,$pDepartmentID = FALSE, $pDivisionID = FALSE,$pProvinceID=0){
	$CI =& get_instance();	
	$sql = "
SELECT SUM(BUDGET) FROM MT_PROJECT_WITHDRAW_DETAIL WHERE ID IN(
...
";
	...
	$sql.= $pProvinceID > 0 ? " AND MPW.PROVINCEID=".$pProvinceID : "";
	$sql.=")";	 
	$result = $CI->db->getone($sql);		
	return $result;
}
function GetSubActWithdraw($pWYear = FALSE,$pWMonth = FALSE,$pSubActID = FALSE,$pDepartmentID = FALSE, $pDivisionID = FALSE,$pProvinceID=0){
	...
	SELECT SUM(TOTAL) FROM (
	SELECT (
	SELECT SUM(BUDGET)TOTAL FROM MT_PROJECT_WITHDRAW_DETAIL WHERE PID=MPW.ID)TOTAL FROM MT_PROJECT_WITHDRAW MPW
	LEFT JOIN MT_PROJECT MP ON MPW.PROJECTID = MP.ID";
	$sql.= " WHERE 1=1 ";
	$sql.= $pWYear > 0 ? " AND  WYEAR=".$pWYear : "";
	$sql.= $pWMonth > 0 ? " AND  WMONTH=".$pWMonth : ""; 
	$sql.= $pSubActID > 0 ? " AND SUBACTID=".$pSubActID : "";
	$sql.= $pDepartmentID > 0 ? " AND DEPARTMENTID=".$pDepartmentID : "";
	if($pProvinceID==2){
	$sql.= $pDivisionID > 0 ? " AND MPW.DIVISIONID=".$pDivisionID : "";
	}
	$sql.= $pProvinceID > 0 ? " AND PROVINCEID=".$pProvinceID : "";
	$sql.=")";
...
function GetProjectWithdraw($pWYear = FALSE,$pWMonth = FALSE,$pSubActID = FALSE,$pProjectID = FALSE,$pMainProjectID = 0 ,$pDepartmentID = FALSE, $pDivisionID = FALSE,$pProvinceID=0){
	...
	$sql.= $pMainProjectID >0 ? " AND MT_PROJECT.ID=".$pProjectID : " AND (MT_PROJECT.PID=".$pMainProjectID." OR MT_PROJECT.ID=".$pProjectID." OR MT_PROJECT.PID=".$pProjectID." ) ";
	$sql.=")";
	//echo $sql;
	//WHERE 
	$result = $CI->db->getone($sql);
...
function CalculateBudgetNet($pMtYear=FALSE,$pBudgetTypeID=FALSE, $pProjectID=FALSE,$pWithdrawID=FALSE,$pProvinceID=0,$pMtMonth=0)
{
	if($pMtYear > 0 && $pBudgetTypeID > 0 && $pProjectID > 0)
	{
		...
		$sql = "SELECT SUM(BUDGET) FROM MT_PROJECT_WITHDRAW mpw LEFT JOIN MT_PROJECT_WITHDRAW_DETAIL mpwd ON mpw.ID = mpwd.PID WHERE mpw.PROJECTID=".$pProjectID." 
		AND BUDGETTYPEID=".$pBudgetTypeID." AND mpw.WYEAR=".$pMtYear." AND mpw.WMONTH <> ".$pMtMonth." AND mpw.PROVINCEID=".$pProvinceID;
...
function GetQuestionairAmount($provinceid,$start_date,$end_date,$type='qty'){
	$CI=& get_instance();
	//$CI->db->debug=true;
	$condition = "";
	$date_condition = "";
	...
	$sql = " SELECT COUNT(*) 
	FROM MT_QUESTIONAIR 	 
	WHERE 1=1 AND PROVINCEID > 0 AND PROVINCEID <> 2 ".$date_condition;
...
function GetQuestionairAnswer($provinceid,$start_date,$end_date,$column_name,$column_value,$type="qty"){
	...
	$sql = " SELECT COUNT(*) 
	FROM MT_QUESTIONAIR 	 
	WHERE 1=1 ".$condition.$excondition;
	$sql = iconv('UTF-8','TIS-620',$sql);
	$nrecord = $CI->db->getone($sql);
...
function GetTargetType($bgyear,$department_id=FALSE,$division_id=FALSE,$province_id=FALSE,$productivity_id=FALSE,$mainact_id=FALSE,$subact_id=FALSE)
{
	if($bgyear > 0){
		$CI=& get_instance();
...
function GetTargetTypeValue($bgyear,$department_id=FALSE,$division_id=FALSE,$province_id=FALSE,$productivity_id=FALSE,$mainact_id=FALSE,$subact_id=FALSE,$target_type_id=FALSE)
{
	if($bgyear > 0){
		$CI=& get_instance();
...
function GetTotalValue($bgyear,$department_id=FALSE,$division_id=FALSE,$province_id=FALSE,$productivity_id=FALSE,$mainact_id=FALSE,$subact_id=FALSE,$budgettype_id=FALSE)
{
	if($bgyear > 0){
		$CI=& get_instance();
...
function GetTargetResult($bgyear,$department_id=FALSE,$division_id=FALSE,$province_id=FALSE,$productivity_id=FALSE,$mainact_id=FALSE,$subact_id=FALSE,$month=FALSE)
{
	if($bgyear > 0){
		$CI=& get_instance();
...
function GetTotalResult($bgyear,$department_id=FALSE,$division_id=FALSE,$province_id=FALSE,$productivity_id=FALSE,$mainact_id=FALSE,$subact_id=FALSE,$month=FALSE,$budgettype_id=FALSE)
{
	if($bgyear > 0){
		$CI=& get_instance();
